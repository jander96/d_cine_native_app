...
class SimilarMoviesViewModel(private val id: Int, private val repo: MoviesRepository) :
    ViewModel() {
    ...
    private val _state = MutableStateFlow(SimilarMoviesState())
    val state: StateFlow<SimilarMoviesState> get() = _state
    private var page by mutableIntStateOf(1)

    suspend fun loadMovies(isRefresh: Boolean = false) {
        if (isRefresh) {
            page = 1
            _state.update { SimilarMoviesState() }
        }
        try {
            _state.update {
                it.copy(
            ...
            val result = repo.getSimilarMovies(id, page)
            _state.update {
                it.copy(
