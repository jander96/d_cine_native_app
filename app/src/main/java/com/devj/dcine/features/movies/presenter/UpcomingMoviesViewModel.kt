...
class UpcomingMoviesViewModel(private val repo: MoviesRepository) : ViewModel() {

    companion object {
        const val PAGE_SIZE = 20
    }

    private val _state = MutableStateFlow(UpcomingMoviesState())
    val state: StateFlow<UpcomingMoviesState> get() = _state
    private var page by mutableIntStateOf(1)

    suspend fun loadMovies(isRefresh: Boolean = false) {
        if(isRefresh) {
            page = 1
            _state.update { UpcomingMoviesState() }
        }
        try {
            _state.update {
                it.copy(
            ...
            val result = repo.getUpcomingMovies(page)
            _state.update {
                it.copy(
